...
class MasterJenisSampahController extends Controller
{
    ...
    {
        try {
            $jenisSampah = MasterJenisSampah::findOrFail($id);

            if ($jenisSampah->harga()->exists()) {
                return response()->json([
                    'error' => 'Gagal! Jenis sampah ini masih digunakan di tabel Harga Sampah.'
                ], 422);
            }
            ...
            return response()->json([
                'error' => 'Terjadi kesalahan saat mencoba menghapus data.'
            ], 500);
        }
    }
}
